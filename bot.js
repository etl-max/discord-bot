...
// ────────────────────────────────────────────────────────────
// Utilise la variable d'environnement configurée sur Render
...
// ────────────────────────────────────────────────────────────
const welcomeStyles = [
  (m) => `☠️ *Un nouveau condamné arrive...* **${m.user.username}** vient de rejoindre. Le destin t'a mené ici... ou peut-être la malletdiction.`,
...
async function handleCommand(command, args, message) {
  ...
  if (command === "unjail") {
    ...
    if (!data) return message.reply("❌ Ce membre n'est pas en jail.");

    const jailCh = guild.channels.cache.get(data.jailChannelId);
    if (jailCh) await jailCh.delete().catch(() => {});
